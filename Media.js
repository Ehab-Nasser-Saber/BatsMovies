//URL data Extraction
let mediaId = location.search.slice(1).split("&");
// console.log(mediaId);

const list = document.querySelector(".contentDisplay");
let mediaTriggers = document.querySelectorAll(
...
document.getElementById("back").addEventListener("click", () => {
  location.href =
    location.origin + "/Info.html?" + mediaId[0] + "&" + mediaId[1];
});
...
async function constructRequest(ID, media_type) {
  ...
  request =
    ...
    addedData +
    "?api_key=47267fe7cba2fdb0a40c15edfb97724c&append_to_response=videos,images";
  // console.log(request);
...
function generatePosterList(data) {
  ...
  for (let item of data) {
    ...
    element.addEventListener("click", () => {
      window.open(
        "https://www.themoviedb.org/t/p/w220_and_h330_face/" + item.file_path,
        "_blank"
      );
    });
...
function generateBackdropList(data) {
  ...
  for (let item of data) {
    ...
    element.addEventListener("click", () => {
      window.open(
        "https://www.themoviedb.org/t/p/original/" + item.file_path,
        "_blank"
      );
    });
    list.appendChild(element);
...
function constructElement(element, key, name, published_at, type) {
  ...
  child1.src =
    type == "video"
      ...
      : type == "poster"
      ? "https://www.themoviedb.org/t/p/w220_and_h330_face/" + key
      : "https://www.themoviedb.org/t/p/original/" + key;
  child1.alt = name;
  ...
  child1.textContent = published_at
    .split("T")
    .join(" ")
    .split(".")
    .splice(0, 1)
