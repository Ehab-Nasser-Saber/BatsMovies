...
const toggledList = document.querySelector(".cast");
const shade = document.querySelector(".darken");
// console.log(shade);
...
let optionList = document.querySelector(".media .optionList");
let options = document.querySelectorAll(".media .optionList h2");
let dropDownArrow = document.querySelector(".media .optionList span");
...
function updateListSize(element) {
  mediaLists[0].element.style =
    mediaLists[1].element.style =
    mediaLists[2].element.style =
      "display: none";
  mediaLists[parseInt(element.id)].element.style =
    "grid-template-columns: repeat(" +
    Math.min(mediaLists[parseInt(element.id)].size, 12) +
    ", " +
    mediaLists[parseInt(element.id)].width +
    "px) 150px;";
}
...
async function checkOnMedia(type, id) {
  const data = await sendRequest(
    "https://api.themoviedb.org/3/" +
      ...
      "/" +
      id +
      "/external_ids?api_key=47267fe7cba2fdb0a40c15edfb97724c"
  );
  // console.log(data);
...
function constructMedia(data) {
  ...
  MakeMediaClickable(
    document.querySelector(".media .horizontalSlider").children,
...
function buildMediaList(data, list, query, includeAnchors, targetIdx) {
  ...
  for (let i = 0; i < sz; i++) {
    ...
    if (includeAnchors == true) {
      ...
      img.src =
        ...
        data[query[0]][query[1]][i].file_path;
      item.appendChild(img);
    }
    item.id = i;
  ...
  setTimeout(() => {
    ...
    spanEl.addEventListener("click", () => {
      location.href =
        location.origin + "/Media.html?" + mediaId[0] + "&" + mediaId[1];
    });
