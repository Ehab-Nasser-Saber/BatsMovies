...
async function controlResults() {
  ...
  if (tv.results.length && movies.results.length)
    topList =
      tv.results[0].vote_average > movies.results[0].vote_average
        ? "tv"
        : "movies";
  ...
  constructOptionList(topList, [
    ...
    {
      name: "Collections",
      number: collections.total_results,
      list: collections,
    },
  ]);
  ...
  for (let trigger of mediaTriggers) {
    ...
    trigger.addEventListener("click", () => {
      mediaTriggers.forEach((element) => {
        element.className = "";
      }); //I clear the classes for the all the options
...
function controlContentList(type) {
  if (type == "People") {
    ...
    switch (type) {
      case "TV Shows":
        if (tv.results.length) constructContentList(tv, "tv");
        break;
      case "Movies":
        if (movies.results.length) constructContentList(movies, "movie");
        break;
      case "Collections":
        if (collections.results.length) constructContentList(collections);
        break;
    }
  }
...
function constructProfileList(data) {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (results[i].profile_path) {
      ...
      imgEl.src =
        "https://www.themoviedb.org/t/p/w90_and_h90_bestv2" +
        results[i].profile_path;
    ...
    for (let j = 0; j < Math.min(results[i].known_for.length, 3); ++j) {
      ...
      if (results[i].known_for[j].title)
        knownFor += results[i].known_for[j].title;
...
function constructContentList(data, type) {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (results[i].release_date)
      pEl.textContent =
        "Released: " + results[i].release_date.split("-").join("/");
    if (results[i].first_air_date)
      pEl.textContent =
        "First Aired: " + results[i].first_air_date.split("-").join("/");
    divEl.appendChild(pEl);
    ...
    item.addEventListener("click", () => {
      location.href =
        location.origin + "/Info.html?" + type + "&" + results[item.id].id;
...
async function constructRequest(type, query) {
  queryStr =
    ...
    type +
    "?api_key=47267fe7cba2fdb0a40c15edfb97724c&query=" +
    query +
    "&page=" +
    pageNumber;
