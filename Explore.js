...
let pageNumber; //Responsible for displaying which page fetched from the database
let optionList; //Responsible for displaying the options from which the user can choose the query (upcoming, popular, etc)
let currentDisplay; //Responsible for indicating which data should be displaying
let displayList; //The element that display the data
let apiData; //Responsible for holding data needed to construct the displayed list
let queryOptions; //Responsible of holding each query option possible as index for easier access (index is sorted according to the optionList)
...
function configureOptionList() {
  for (let i = 0; i < 4; i++) {
    optionList[i].id = i;
    optionList[i].textContent = queryOptions[i]
      .split("_")
...
function constructList() {
  ...
  for (let i = 0; i < dataList.length; i++) {
    ...
    item.className = "item";
    if (mediaType == "people") item.className += " profile";
    item.id = i;
    ...
    if (dataList[i].release_date)
      pEl.textContent =
        "Released: " + dataList[i].release_date.split("-").join("/");
    if (dataList[i].first_air_date)
      pEl.textContent =
        "First Aired: " + dataList[i].first_air_date.split("-").join("/");
...
//Responsible of configuring the page numbers at the bottom of the page
...
//Responsible of constructing the page number at the bottom of the page and giving them functionalities
...
//Responsible for sending the user all the way to the top of the page
function sendUserToTop() {
  scroll({ top: 0, behavior: "smooth" });
}

//Responsible for sending information to construct request and assign the api data to the constiable apiData
...
async function constructrequest(media_type, list_type) {
  query =
    ...
    "/" +
    list_type +
    "?api_key=47267fe7cba2fdb0a40c15edfb97724c&page=" +
    pageNumber;
  return await sendRequest(query);
}

//Responsible for calling the API and returning the raw data from the call results
